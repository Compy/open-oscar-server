...
func (s *Server) handleConnection(conn net.Conn) {
	...
	s.logger.Info("federation inbound peer authenticated",
		"peer", networkName, "remote", conn.RemoteAddr())
...
func (s *Server) performInboundAuth(flapc *wire.FlapClient) (string, error) {
	...
	if !ok {
		...
		}, wire.SNAC_0x0100_0x0003_FedAuthResult{Code: wire.FedAuthResultUnknown})
	...
	// Send our response to their challenge
	...
	if err := flapc.SendSNAC(wire.SNACFrame{
		...
		NetworkName: s.manager.LocalNetwork(), Digest: digest,
	}); err != nil {
		return "", err
	}

	// Receive their response to our challenge
	flap, err = flapc.ReceiveFLAP()
	if err != nil {
